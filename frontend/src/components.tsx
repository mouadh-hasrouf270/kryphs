...
import {api,post,label,csrf,type Item,type Page,type Field,type Schema} from './api'
import {useI18n} from './i18n'

export function ErrorBox({error}:{error:unknown}) {const {t}=useI18n();return <div className="error" role="alert"><AlertCircle size={18}/><div><strong>{t('error')}</strong><p>{error instanceof Error?error.message:String(error)}</p></div></div>}
...
export function RelationField({field,workspace,value,onChange}:{field:Field;workspace:string;value:unknown;onChange:(v:unknown)=>void}){
 const {t}=useI18n();const query=useQuery({queryKey:['options',field.related,workspace],queryFn:()=>api<Page>(field.related+'/',workspace)})
...
export function RecordForm({resource,workspace,schema,initial,defaults={},onDone}:{resource:string;workspace:string;schema:Schema;initial?:Item;defaults?:Record<string,unknown>;onDone:()=>void}){
 const {t}=useI18n();const client=useQueryClient();const formDefaults=Object.fromEntries((schema[resource]?.fields||[]).filter(f=>f.default!==null&&f.default!==undefined).map(f=>[f.name,f.kind==='json'?JSON.stringify(f.default):f.default]));const {register,handleSubmit,setValue,watch}=useForm<Record<string,unknown>>({defaultValues:initial||{...formDefaults,...defaults}})
 ...
 {f.kind==='relation'||f.kind==='multi'?<RelationField field={f} workspace={workspace} value={watch(f.name)} onChange={v=>setValue(f.name,v)}/>:
...
export function ResourceList({resource,workspace,schema,title,filters={},onOpen,compact=false}:{resource:string;workspace:string;schema:Schema;title?:string;filters?:Record<string,string>;onOpen?:(item:Item)=>void;compact?:boolean}){
 ...
 {selected&&!operation&&<Modal title={label(selected)} close={close}><div className="detail-content"><div className="detail-grid">{Object.entries(selected).filter(([k,v])=>v!==null&&v!==''&&!['id','workspace'].includes(k)).map(([k,v])=><div key={k}><dt>{t(k)}</dt><dd>{typeof v==='object'?JSON.stringify(v,null,2):String(v)}</dd></div>)}</div><div className="action-row">{schema[resource]?.can_edit&&<button className="button primary" onClick={()=>{setEditing(selected);setSelected(undefined)}}>{t('edit')}</button>}
 ...
 {resource==='ad-connections'&&<button className="button secondary" onClick={()=>setOperation('credentials')}>{t('credentials')}</button>}
 {resource==='ad-connections'&&<button className="button secondary" disabled={selected.provider!=='meta'} onClick={()=>setOperation('sync')}>{t('sync')}</button>}
 ...
 {resource==='jobs'&&selected.status==='failed'&&<button className="button secondary" onClick={()=>setOperation('retry')}>{t('retry')}</button>}
