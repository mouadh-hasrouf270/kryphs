...
test('cross-workspace and restricted-brand reads are rejected in browser sessions',async({page})=>{
 ...
 const me=await call(page,'me/');const ws=me.data.workspaces.find((w:{name:string})=>w.name==='Olive Studio').id;const original=await call(page,'brands/?workspace='+ws);const allowed=original.data.results.find((b:{slug:string})=>b.slug==='noura')
