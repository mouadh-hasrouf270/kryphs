...
test('Drive development fixture retains identity through rename and move',async({page})=>{
 const python=process.platform==='win32'?'../backend/.venv/Scripts/python.exe':'../backend/.venv/bin/python';const script='../backend/tests/seed_drive_fixture.py'
 ...
 await page.goto('/integrations');await expect(page.getByRole('button',{name:'Renamed fixture.png',exact:true}).first()).toBeVisible()
})
