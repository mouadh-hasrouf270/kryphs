...
test('complete request, deliverable tasks, revision, quantity and parent approval',async({page})=>{
 ...
 for(let i=1;i<=2;i++){await dialog.getByRole('button',{name:'Add deliverable',exact:true}).click();const child=dialog.locator('.deliverable-input').nth(i-1);await child.getByLabel('Title',{exact:true}).fill('Deliverable '+i);await child.getByLabel('Quantity',{exact:true}).fill(i===1?'1':'3')}
 ...
 for(const [i,other] of others.entries()){expect((await call(page,`creatives/${other}/actions/approve/`+suffix,'POST',{})).status).toBe(200);if(i===0)expect((await call(page,`deliverables/${children[1].id}/`+suffix)).data.status).not.toBe('approved')}
